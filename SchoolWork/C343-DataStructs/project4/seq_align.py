...
def makestr(ls, s1, s2):
    ls.reverse()
    str1 = s1
    str2 = s2
    for i in range(len(ls)):
        if ls[i] == "l":
            str2 = insert_gap(i, str2)
        if ls[i] == "u":
            str1 = insert_gap(i, str1)
        else:
            pass
    return str1, str2
...
def seq_align(s1, s2, enable_graphics=True):
    ...
    for i in range(0, len(s3)):
        for j in range(1, len(s4)):
            if i == 0:
                noChange = s(SPACE_CHAR,s4[j]) + board[(0,j-1)]
                board[(i,j)] = noChange
                board[(j,i)]= noChange
                #print s3[i], s4[j], noChange
            else:
                corner = s(s1[i-1],s2[j-1]) + board[(i-1,j-1)]#-2
                left = s(SPACE_CHAR,s2[j-1]) + board[(i-1,j)]#-1
                up = s(SPACE_CHAR,s2[j-1]) + board[(i,j-1)] #-1
               # print board[i-1][j-1], 'board at i-1, j-1'
               # print 'nochange', s(s1[i],s2[j])
